 ...
 * <p>
 * Companions will typically share the same {@link Costume} as their source, but
 * will use a different {@link Pose}.
 */
public abstract class Companion extends AbstractRole {
	...
	public static abstract class AbstractCompanionBuilder<C extends Companion, B extends AbstractCompanionBuilder<C, B>> {
		 ...
		 * <p>
		 * For example, imagine we have a costume called "bigShip". Create a
		 * string within bigShip, named "bullet" with a value "redBullet". We
		 * can now create a bullet like so :
		...
		public B companion(String companionType) {
			Costume costume = this.companion.source.getCostume().getCompanion(
					companionType);

			if (costume == null) {
				// Fall back to the old-fashioned way of doing it.
				costume = Itchy.getGame().resources.getCompanionCostume(
						this.companion.source.getCostume(), companionType);
			}
		 ...
		 * instead select a Pose from the source's costume using
		 ...
		 * {@link #eventName(String)} instead, as it can also initiate an
		 ...
		 * not used. Can be used in conjunction with
		 ...
		 *            used in conjunction with {@link #offset(double, double)}
		 *            and {@link #offsetForwards(double)}.
		 * @return this
		 */
		 ...
		 * the source actor, or it can be overridden by
