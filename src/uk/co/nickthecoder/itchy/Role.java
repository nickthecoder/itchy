...
/**
 * Determines how an Actor behaves, including how it moves and interacts with other Actors. Role is arguably the most important part of
 ...
 * Java or Python then you probably want to use {@link AbstractRole} as the base class for all of your Roles.
 * <p>
 ...
 * Roles is when Pacman eats a power pellet, the enemies change Role from predators, to prey (the blue ghosts), and when they are eaten they
 * change again (a ghostly pair of eyes returning to their home).
 * <p>
 * Roles have tags, each tag is just a String, but used to group sets of actors together. Tags are most often used when checking for
 * collisions. For example, a bullet may check if it is colliding with an Roles which have a "shootable" tag.
 */
public interface Role extends MessageListener, Cloneable, PropertySubject<Role>
{
    ...
    /**
     * Adds a tag to the Role. Tags behave as a Set, so adding the same tag twice has no effect, and if you add a tag twice, and then remove
     * it once, then it will not have that tag.
     * 
     * @param tag
     *        The string for this tag - normally a single word, such as "shootable".
     */
     ...
     * actor to all other actors using a pixel based collision test ({@link BruteForceCollisionStrategy#pixelCollision}). Brute force is
    ...
    /**
     * Called when the actor has both a role, and is on a stage. The order may vary: when added to a stage, and then given a role, it is
     * called from Actor.setRole, but when the role is set first, it is called when the actor is added to a stage.
     * <p>
    ...
    /**
     * Called when a difference role is assigned to this Role's Actor. It is not called when the actor dies.
     * <p>
     * Should only be called internally by Itchy.
     */
    void detatched();
    ...
    /**
     * Called when a message is sent to this Role. Messages are a simple way to communicate with a Role, but it is often better to create a
     * specific method, rather than passing strings to onMessage.
     */
    @Override
    ...
    /**
     * Returns the list of properties which this type of Role has. For pure Java Roles, the properties can be automatically created using
