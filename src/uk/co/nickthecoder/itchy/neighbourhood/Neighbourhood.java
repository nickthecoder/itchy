...
public class Neighbourhood
{
    ...
    /**
     * Looks for a NeighbourhoodSquare within the neighbourhood. If a square at
     * the given coordinates hasn't been created yet, then that square is
     * created.
    ...
    /**
     * Looks for a NeighbourhoodSquare within this neighbourhood. If a square at
     * the given coordinates hasn't been created yet, then null is returned.
     */
    ...
    private NeighbourhoodRow createRow( double y )
    {
        ...
        if ( iy < 0 ) {
            ...
            for ( int i = 0; i < extra; i++ ) {
                //System.out.println( "Creating row at tail " + ( this.oy + this.rows.size() * this.squareSize ) );
                NeighbourhoodRow row = new NeighbourhoodRow( this.oy + this.rows.size() * this.squareSize );
                this.rows.add( row );
            }
    ...
    public class NeighbourhoodRow
    {
        ...
        private Square createSquare( double x )
        {
            ...
            if ( ix < 0 ) {
                ...
                for ( int i = 0; i < -ix; i++ ) {
                    newSquares.add( new Square( Neighbourhood.this, this.ox + i * Neighbourhood.this.squareSize, this.y ) );
                }
                this.row.addAll( 0, newSquares );
                for ( int i = 0; i < -ix; i++ ) {
                	//System.out.println( "Index " + i + " sizes " + newSquares.size() + " & " + this.row.size() );
                	//System.out.println( "   new square " + ( newSquares.get( i ) == this.row.get( i ) ) );
                ...
                for ( int i = 0; i < extra; i++ ) {
                    Square square = new Square( Neighbourhood.this, this.ox + (this.row.size() ) * Neighbourhood.this.squareSize, this.y );
                    this.row.add( square );
                    square.initialise();
                }
    ...
    public void debug()
    {
    	System.err.println( "Neighbourhood : " + this.getSquareSize() + " oy=" + oy );
    	double y = oy;
    	for ( NeighbourhoodRow row : rows ) {
    		System.err.println( "\nRow : " + y  + " ... " + row.y );
